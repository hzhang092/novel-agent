...
def test_context_round_trip_and_overwrite(tmp_path):
    """Facts and summaries round-trip; saving facts replaces the prior set."""
    ...
    replacement = [CanonFact(description="事实B", category="plot", source_scene_id="s2")]
    save_canon_facts(proj_dir, replacement)
    assert load_canon_facts(proj_dir) == replacement
    ...
    summaries = [
        ...
        SceneSummary(scene_id="scene-2", chapter_id="ch-1", summary="林轩展现实力"),
    ]
    save_scene_summaries(proj_dir, summaries)
    assert load_scene_summaries(proj_dir) == summaries


def test_empty_context_files_return_empty_lists(tmp_path):
    from app.storage.project_files import load_canon_facts, load_scene_summaries

    proj_dir = create_project(tmp_path, Project(title="测试", genre="玄幻"))
    assert load_canon_facts(proj_dir) == []
    assert load_scene_summaries(proj_dir) == []
